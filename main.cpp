...
double* getColor(unsigned char a, unsigned char b, unsigned char c) {
...
int main(int argc, const char** argv) {
  ...
  bool toMovie = true, noOutput = false;
  bool png = true;
  for (int i = 1; i < argc; i++) {
    ...
    if (streq(argv[i], "--help")) {
      printf("Usage %s [-H <height>] [-W <width>] [-F <framecount>] [--movie] [--no-movie] [--no-output] [--png] [--ppm] [--help] [-o <outfile>] [-i <infile>]\n", argv[0]);
      return 0;
    }
